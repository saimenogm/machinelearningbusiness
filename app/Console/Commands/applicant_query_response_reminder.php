...
class applicant_query_response_reminder extends Command
{
     ...
     * @var string
     */
    protected $description = 'Reminder applicant to submit query response 5 days prior to deadline';
    ...
    public function handle()
    {
        ...
        foreach ($assignments as $assignment) {
            ...
            if ($assignment->received_date == null) {
                ...
                if ($diff_in_days <= $reminder_limit and $diff_in_days >= 0) {
                ...
                } elseif ($diff_in_days == -1) { //expired but response is still not sent
                    ...
                    queries::where('query_related_id', $assignment->dossier_assign_id)->update([
                        'status' => 'Locked'    //todo mera: disable upload button for applicant
                    ]);
