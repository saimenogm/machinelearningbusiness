...
<div  class="modal fade" id="ajax_model_issue_query" aria-hidden="true">
...
<form method="POST" enctype="multipart/form-data" id="IssueQForm"     method="POST" name="upload_first_form"   action="{{ route('application.IssueQuery_front')   }}" 
 ...
 accept-charset="utf-8" class="form-horizontal"  >
              ...
              <input type="hidden" name="_token" value="{{ csrf_token() }}" />
                    ...
                    <select class="form-control select2bs4" id="number_application"  name="number_application"  required>
                                    <option value="0" disable="true" selected="true">Select Application Number</option>
                                    @foreach($applications_front as $value)
                                   <option value="{{$value->application_id}}">{{ $value->application_number }}</option>
                                    @endforeach
                                    </select>
                        </div>
