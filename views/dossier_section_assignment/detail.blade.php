                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-6 col-lg-6 col-sm-6">
                                    <label> Assigned By (Assessor Name):</label> {{ $assigned_section->first_name }} {{
                                $assigned_section->middle_name }} <br>
                                    <label> Subject: </label> {{ $assigned_section->assignment_description
                                }}<br>
                                    <label> Assignment Date: </label> {{
                                $assigned_section->section_sent_date }}<br>
                                    <label> Deadline: </label> {{
                                $assigned_section->section_deadline }}<br>
                                    <label> Remaining Days: </label>
                                    {{$diff_in_days}}
                                    <br>
                                    <label> Document to be Assessed: </label> <a
                                        href="{{ asset($assigned_section->sent_document_path)}}"
                                        target="_blank" data-toggle="tooltip" class="btn btn-info btn-sm"
                                        data-placement="top"
                                        title="View the file"><i class="fas fa-book-open"></i></a> <br>
                                </div>
                                <div class="col-md-6 col-lg-6 col-sm-6">
                                    @if ($assigned_section->section_received_date!=null)

                                        <label> Subject: </label> {{ $assigned_section->response_description
                                    }}<br>
                                        <label> Response Date: </label> {{
                                    $assigned_section-> section_received_date}}<br>

                                        <label> Response Document : </label> <a
                            ...
                            <div class="modal-footer justify-content-between">
                                ...
                                @if ($assigned_section->section_received_date==null)
                                    <button type="button" class="btn btn-warning" title="Request
                                    ...
                                    the evaltuation to the Assessor"
                                ...
                                @else
                                    <button data-toggle="modal" data-target="#editSectionResponseModal"
                                        ...
                                        <i class="fas fa-edit"></i>Reupload Response
                                    </button>
                                @endif
                            </div>
                        </div>
            ...
            <div class="modal fade" id="submit_evaluation_to_assessor" data-backdrop="static" tabindex="-1"
                ...
                <div class="modal-dialog modal-md" role="document">
                    ...
                    <form action="{{ route('dossier_section_upload') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title">Upload Dossier Section
                                    Evaluation Document</h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
