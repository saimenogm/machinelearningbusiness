    ...
    public function handle()
    {
        ...
        foreach ($assignments as $assignment) {
            ...
            if ($assignment->received_date == null) {
                ...
                } elseif ($diff_in_days == -1) { //expired but report is still not uploaded

                    // on expire, disable report uploading (user has to request extension)
                    dossier_section_assignment::where('section_related_id', $assignment->dossier_assign_id)->update([
                        'status' => 'Locked'    //todo mera: disable upload button for units
                    ]);
